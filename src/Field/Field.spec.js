...
describe("Field", () => {
  ...
  //   ]
  // }
  it("Render text Field with default options", () => {
  ...
  it("Render text Field with default options", () => {
    const component = shallow(
      <Field
        ...
        onError={(errorObj) => {
          console.log(errorObj.error, errorObj.errorMessage);
        }}
      />
    );
    const tree = shallowToJson(component);
  ...
  it("Render number Field with default options", () => {
    const component = shallow(
      <Field name="number" type="number" required={true} defaultValue={5} />
    );
    const tree = shallowToJson(component);
    expect(tree).toMatchSnapshot();
  });
